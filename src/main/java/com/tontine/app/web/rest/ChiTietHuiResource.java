...
public class ChiTietHuiResource {
    ...
    public ResponseEntity<ChiTietHui> updateChiTietHui( @PathVariable Long id, @RequestBody ChiTietHui chiTietHui ) {
        validateChiTietHuiId( id, chiTietHui );

        ChiTietHui result = chiTietHuiService.update( chiTietHui );
        return ResponseEntity.ok()
            .headers( HeaderUtil.createEntityUpdateAlert( applicationName, false, ENTITY_NAME,
                                                          chiTietHui.getId().toString() ) )
            .body( result );
    }

    @PatchMapping( value = "/chi-tiet-huis/{id}", consumes = {"application/json", "application/merge-patch+json"} )
    public ResponseEntity<ChiTietHui> partialUpdateChiTietHui( @PathVariable Long id,
